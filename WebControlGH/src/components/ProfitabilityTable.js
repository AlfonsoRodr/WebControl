...
function ProfitabilityTable() {
  ...
  const data = [
    { cod: '0009', descripcion: 'Gastos Generales', tipo: 'INST+PPM', fComienzo: '09-08-2016', fFinal: '21-08-2019', importe: 0, hPrev: 0, hTotal: 90, gReal: 71673.72, rent: -71673.72, pRent: 0.00 },
    { cod: 'SAT719', descripcion: '4101467811 FRANCE TELECOM SEDE ULISES', tipo: 'SAT TR', fComienzo: 'Sin Horas', fFinal: 'Sin Horas', importe: 0, hPrev: 0, hTotal: 0, gReal: 0.00, rent: 0.00, pRent: 0.00 },
    { cod: '2351RO', descripcion: 'Sede Nacional Testigos Jehova WATCH TOWER', tipo: 'PPM', fComienzo: '29-01-2018', fFinal: '29-10-2020', importe: 25295, hPrev: 670, hTotal: 768.5, gReal: 16649.67, rent: 8745.33, pRent: 34.44 }
  ];
    ...
    <div className="profitability-table-container">
          ...
          <table className="profitability-table">
            ...
            <tbody>
              {filteredData.map((item, index) => (
                <tr key={index}>
                  <td><input type="checkbox" checked={selectedItems.includes(item.cod)} onChange={() => handleSelectItem(item.cod)} /></td>
                  <td>{item.cod}</td>
                  ...
                  <td>{item.rent}</td>
                  <td>{item.pRent}</td>
                  <td><button>Detalles</button></td> {/* Añadido botón de detalle */}
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <div className="totals-and-actions">
            <div className="actions">
              <div className="action-button">
                <button onClick={handleRecalculateClick}>Recalcular Rentabilidad Obras</button>
      ...
      {showConfirmation && (
        <div className="confirmation-modal">
          <p>¿Estás seguro de que deseas recalcular la rentabilidad de las obras seleccionadas?</p>
